...
Li Junnan1, Yang Xiangrui, Sun Zhigang1 
...
摘要公有云中需要灵活地部署网络功能以实现多租户间的网络隔离、服务质量保证与安全防护。软件实现的网络功能具有较高的灵活性，但性能较低，存在分组处理吞吐率较低，处理延时较高等不足。FPGA具有良好的可编程性及较高的处理性能，但如何使用FPGA实现可重构的网络功能缺乏统一的架构和方法。
...
传统基于专用硬件的网络功能实现方法灵活性差，无法满足快速部署网络功能的需求[4]。为此，目前主流云服务商，如微软，亚马逊，主要采用软件实现各类网络功能，以获得最大的灵活性[1,5]。然而软件实现的网络功能存在两方面不足。一是处理性能有限，通常需要多个CPU核并性处理才能达到10 Gbps速率[1,7]，难以满足40Gbps甚至更高速率[8]的处理需求。二是软件处理的延时具有较大不确定性，延时从几十微秒到几毫秒不等[9]，这对于许多低延迟应用而言是难以接受的[10]。
...
基于FPGA的网络功能实现方式面临的主要问题是硬件编程困难，与GPU和NP的软件编程相比，开发周期长。为了简化硬件开发，近年来基于高级语言的FPGA网络功能开发技术得到广泛研究[1,17-18]，包括支持开发人员使用高级语言（例如C/C++）编程的高级综合工具(HLS)[19-20]，或者直接使用特定域语言(domain-specific language)，如P4[22]，描述网络处理功能[21]，然后通过编译器将其映射到FPGA逻辑[23-24]。然而，上述研究还难以达到实用的效果，主要是因为高级语言并不涉及寄存器级描述，而且高级综合工具仍无法很好地解决数据相关问题（例如读写依赖）。同时，由高级语言编译产生的硬件代码还存在资源开销大，处理性能低等不足[25]。
我们认为，除了使用高级语言编程以外，通过设计模块化的分组处理流水线，基于标准的模块设计和模块复用也可以简化FPGA开发的复杂性，缩短开发周期。为此，我们提出了可重构流水线模型——DrawerPipe。该模型将网络功能实现架构抽象为5个标准的“抽屉”，不同的“抽屉”可以根据需要装载不同的处理模块，通过组合这些处理模块实现各种网络功能。
...
（2）针对模块的可重用需求，设计了功能无关的“抽屉”之间的信息交换接口——DrawerMPI。DrawerMPI采用“强语法，弱语义”的信息交换模型，通过语义适配模块将上下级“抽屉”中的模块解耦，使得第三方开发的模块不再依赖于上下游模块的接口定义；
...
本文第1节介绍了研究背景与动机，包括在FPGA上部署网络功能的需求，与基于FPGA可重构流水线模型的相关研究。第2节分析了公有云中常部署的11种网络功能的处理流程，并将其抽象到一种通用的可重构流水线中。第3节详细描述了可重构流水线模型的设计思想和工作模型。并在第4节介绍了功能无关的可编程模块接口设计。第5节阐述了基于DrawerPipe实现多种不同的网络功能，并对其进行相应的性能测试。最后是对全文的总结，并对下一步的工作进行展望。
1	研究背景
1.1网络功能动态部署的需求
...
图1 数据中心的三种特殊网络功能
络功能，实现租户特定的分组处理需求[3]。例如，由于IP组播缺乏可靠的数据包传播、流量控制和可扩展性，数据中心本身没有开启组播协议[27]。但租户的一些分布式计算或系统备份任务[28-29]需要同步数据，即将各自的状态、数据信息组播或者广播给该租户的其它虚拟机。如果采用单播方式实现组播功能，需要复制多份相同的数据包送给目标虚拟机，会占用额外的链路带宽。如1-b展示了在交换机端部署自定义转发功能，能够节省发送带宽。另外，一些网页搜索服务通常采用分派--聚集模型[30]，即向多个服务器发送查询请求，再将返回的响应信息聚合得到最终的查询结果；或者分布式计算架构采用映射--规约(map-reduce)模型[28]。在主机端实现聚合或规约存在incast问题，即多个响应报文同时返回给请求主机导致链路拥塞。图1-c展示了在交换机侧部署数据聚合功能[3]，以有效缓解incast导致的链路拥塞。
1.2 相关研究
在FPGA上部署网络功能可以解决软件实现网络功能处理性能低，处理延时高的不足。但由于租户对网络功能的需求随着自身所部署的应用类型、计算任务发生变化，要求FPGA具备灵活快速部署网络功能的能力。
为了避免新网络功能开发与测试周期长的不足，不少的工作[1,7,13,25]借鉴Click[31]路由器模块化设计的思想，并通过共享硬件模块使开发者使用已有的模块重新组合分组处理逻辑。但由于不同分组处理流水线在功能划分、模块接口定义存在差异，不同模块间无法直接互联，还需要统一的可重构流水线模型。为此，相关研究[34-35]提出了基于FPGA的可重构流水线模型，即将流水线分解为不同的处理阶段，通过在每一处理阶段加载不同的功能模块重构新的网络功能。
目前的可重构流水线模型有动态可重构[34]和静态可重构[35]两种方式。动态可重构是预先在FPGA上实现多种基本功能模块。然后根据分组处理需求，动态编排数据包经过模块的次序，实现所需的分组处理逻辑。这种方式极大地提升了FPGA的灵活性，只需要重新编排模块顺序即可实现新的网络功能。但FPGA资源有限，能够同时在FPGA上加载的模块数量较少，因此只能支持少量的网络功能。
...
一方面，FPGA内部是大量同构的可编程逻辑块组成，FPGA开发工具将用户编写的逻辑映射到这些逻辑块中，因此不同功能模块间逻辑资源的分配是在代码综合过程中实现的。这种方式与ASIC或CPU实现相比，具有两个优点。一是模块之间的资源是预分配的，在分配时所有模块共享FPGA的所有资源，因此比RMT需要将逻辑映射到固定的32级流水线上更简单。此外，一旦分配成功，其处理性能是有保证的，而且访问外部存储器完全由有限状态机控制，没有cache层次，因此性能是有保证的。
...
图2不同的流水线模型对比
各自独占内部的存储资源，避免了抽屉之间依赖关系；
（3）“抽屉”之间具有良好的可移植性。规格相同的“抽屉”之间可以相互替换。对模块而言，定义相同的接口模块直接也可以相互替换。
...
DrawerPipe流水级模型，由报文解析，报文分类，自定义网络功能，报文重组，输出控制5级组成，如图3所示。同时，为了避免报文体经过所有模块造成模块存储资源的浪费，我们还设计了单独的报文缓存模块用于存储报文。功能模块间使用Metadata作为信息交互，Metadata采用标准的语法定义（即具有确定的数据位宽），而非采用统一的语义（即定义metadata中每一位数据所代表的含义）。这种协议无关的抽象能够解除模块间由接口信号定义而绑定的耦合性，将在下一节中详细介绍。
3.3 可重构模块库
可重构功能模块库能够为DrawerPipe提供分组处理的功能模块。每一阶段均有其对应的功能模块库，方便用户从中挑选合适的模块，并将它们重组成所需的分组处理逻辑。例如，为了实现二层交换与ACL过滤功能，我们需要在解析层加载了四层报文解析器（L4 Parser），用于提取报文的5元组信息，并分离报文头与报文负载。于此同时，将报文负载存储在负载缓存模块中；而将报文头以及解析器提取的关键字段组成报文控制块（Metadata），送给报文分类模块。在报文分类层加载基于BV的报文分类算法，实现ACL过滤功能。在自定义功能层加载二层交换功能，实现源MAC地址查找，目的MAC地址转发功能。然后在报文重组模块与报文负载重组，最后经输出控制模块从网络设备端口输出。
...
4	DrawerMPI设计
目前的模块设计是协议相关的，即模块的接口定义与模块所处理的协议类型相关。而模块的接口信号定义直接影响模块间是否能够重组。进一步分析，模块的接口信号定义类似于自然语言，也可以将其分成语法与语义两部份。模块接口的语法是指接口信号的位宽、以及输入输出接口的信号数量；而模块接口的语义则是指信号所代表的特殊含义。因此，协议相关的模块接口设计导致处理不同协议的模块之间在语法或者语义上存在差异，无法直接重组构建新的分组处理流水线。
本文提出了DrawerMPI（DrawerPipe Module Programmable Interface），一种协议无关的可编程模块接口，用于解除模块间由接口信号定义而绑定的耦合性。协议无关是指DrawerMPI采用语法一致的抹开接口（§4.1），包括输入输出接口数量、信号位宽，并利用语义转化模块（§4.2）实现模块接口语义转化。DrawerMPI本身不关心上下游模块所处理协议的类型（或模块接口定义）。DrawerMPI还具有编程能力，通过配置接口信号语义的转化规则，实现不同模块接口之间的适配。因此，对网络功能开发者而言，无须知道模块间的信号定义，只需通过配置语义转化模块即可实现模块信号格式的转化。
4.1 DrawerMPI语法设计
模块间能够互连的基本条件是具有模块接口语法相同，包括接口数量、位宽。为此，DrawerMPI采用语法一致的模块，即使用固定位宽的报文控制块（Metadata）作为模块间通信的消息，如图3-b所示。Metadata作为模块间交互消息的好处是位宽恒定，保证了所有模块具有相同的接口。DrawerPipe所使用的Metadata位宽为132位，其中包括header tag4位，分别是mht (metadata header tag)用于标识Metadata首部，met (metadata header end)标识metadata尾部，pht (packet header tag)标识报文头首部，phe(packet header end)标识报文头尾部，随后的128bit是具体的报文控制块内容。另外，DarwerPipe使用报文控制块连接模块，即上游模块只需要往下游模块队列中写控制块信息，而下游模块只需要从队列中读取控制块信息，从而够保证所有模块都具有单输入、单输出接口的特性，便于模块重组。
对报文控制块进一步分析，我们发现模块间对通信内容可以分为三种类型的消息，即平台相关消息，通用报文头字段消息，模块间中间状态交互消息（后两者为平台无关消息）。其中，平台相关消息是指平台自身提供的内容，包括报文输入端口、输出端口、报文时间戳等信息。通用报文头字段消息是指由解析模块提出产生的关键字信息，可以用于查表操作。由于关键字具有固定格式，在不同的语义环境下可以通过转化进行适配(§4.2)。模块间中间状态交互消息是指协同工作的模块间所传递的中间状态消息。例如，源MAC地址学习与目的MAC地址转发功能分离的两个模块，在源MAC地址学习到新的MAC-Port表项后需要将该信息（中间状态信息）告诉目的MAC地址转发模块。在需要相互协同的模块设计之处，开发者已定义好中间状态通信格式，具有统一的语义，无须第三方加以转化。这对上述三种类型消息，DrawerMPI所定义的Metadata采用三层消息模型，分别为平台相关消息层、通用报文头字段消息层、模块间中间状态交互消息层。
...
模块，实现控制块的适配功能。
语义转化模块使用多路选择器（在verilog中采用case语句实现多路选择器）完成metadata格式转化功能，如图3-c所示。我们发现所支持的metadata格式转化粒度越细粒度（例如，支持1bit的syn_tag语义转化），灵活性越高，但消耗的硬件资源也越多。为了在灵活性与资源开销得到较好的折中，我们选择8bit作为最小拼接单元。算法1描述了语义转化模块处理流程。
算法 1：模块接口语义转化
1
...
7
        SelectCode[offsetNext/8]  offsetPre/8
4
      end if
5
    end for
...
5	实验与测试
为了评估该模型的有效性、性能与资源开销，我们基于DrawerPipe模型在Netmagic[54] (Altera FPGA)平台上实现了§1.1中提到的2种网络功能，即自定义转发、数据聚合功能。表2给出了这两种网络功能所使用的一些关键功能模块及模块信息，包括模块的吞吐率，处理性能，资源开销等。可以发现DrawerPipe的模块化设计可以很大程度提高代码的重用，并简化新网络功能的构建。例如，这两种网络功能均使用了L4_Parser，以及基于bv的报文分类算法。
5.1 性能测试
5.1.1 自定义转发功能
...
为了验证基于DrawerPipe实现自定义转发功能的有效性，我们对比了加载自定义转发功能与未加载该功能两种通信方式。其中在未加载自定义转发功能时，测试主机需要与通信对端主机簇建立一对一连接。而基于DrawerPipe实现自定义转发功能，则只需要与虚拟的主机簇地址建立一条连接。
在实验测试中，我们通过一台主机向多台主机发送相同数据。但受Netmagic端口数量的限制，最多测试了1 to 4的情况。如图5所示，我们测试了传输不同大小数据所需的传输时间，即流完成时间。由于传输不同大小数据的流完成时间相差较大，我们将其标准化处理（以部署自定义转发功能所需的流完成时间作为参考基数，即为“1”）。可以发现，在传输较小数据流时（），单播与自定义转发两种方式传输时间相差较小。这是因为发送的报文数量较少（），绝大部分时间为传输延时。而在在传输较大数据流时（），自定义传输方式能节省大量的传输时间，并且通信对端主机数量越多，效果越明显。
...
为了验证网络功能的有效性，我们比较了部署数据聚合功能与传统未部署聚合功能在传输不同数据大小下的流完成时间，如图6所示。实验结果显示，在传输较小数据流时（），未部署数据聚合功能的流完成时间只是稍大；而在传输的数据增大时（），流完成时间急剧上升。这是因为多台主机同时发送报文，交换机的队列溢出而发生丢包（即incast问题[30]），导致主机需要重传丢失的报文。在通信数据更大时，多个主机通过平分带宽避免拥塞，流完成时间与通信主机数量相关。
...
5.2 资源开销分析
我们测试了DrawerPipe同时实现的2种功能网络功能所需的硬件资源开销，包括功能模块，Metadata Adapter，以及平台相关资源开销三部分，如表3所示。平台相关部分大概占用了一半的硬件资源(35.8%~59.1%)，功能模块的资源开销相对较小(11.9%~20.9%)，Metadata Adapter的资源开销也相对较少(2.6%~3.5%)。总结
本文提出了可重构分组处理流水线模型——DrawerPipe。DrawerPipe将网络功能抽象成通用的五级流水线模型，每一级实现某一类分组处理功能。通过为每一级流水线加载一个或者多个类似的功能模块快速重构所需的分组处理逻辑。采用模块重组的方式不仅具有良好的处理性能与高效的资源利用率，同时避免重新开发分组处理流水线，节省开发时间和成本。本文还提出了一种协议无关的模块接口模型——DrawerMPI。DrawerMPI采用语法一致的接口信号定义，而放宽对接口语义的定义，有利于提高模块复用。我们为每一级流水线设置相应的功能模块库，方便用户从每一级模块库中选择并重构所需的处理逻辑。
...
[21]  	Wang H, Soulé R, Dang H T, et al. P4FPGA: A Rapid Prototyping Framework for P4[C]//Proceedings of the Symposium on SDN Research. ACM, 2017: 122-135. Lavasani M, Dennison L, Chiou D. Compiling high throughput network processors[C]//Proceedings of the ACM/SIGDA international symposium on Field Programmable Gate Arrays. ACM, 2012: 87-96.
[22]  	Bosshart P, Daly D, Gibb G, et al. P4: Programming protocol-independent packet processors[J]. ACM SIGCOMM Computer Communication Review, 2014, 44(3): 87-95.
...
[24]  	Chole S, Fingerhut A, Ma S, et al. dRMT: Disaggregated Programmable Switching[C]//Proceddings of the Confrence of the ACM Special Internet Group on Data Communication. ACM, 2017: 1-14.
...
Li T, Sun Z, Jia C, et al. Using NetMagic to observe fine-grained per-flow latency measurements[C]//ACM SIGCOMM Computer Communication Review. ACM, 2011, 41(4): 466-467.
